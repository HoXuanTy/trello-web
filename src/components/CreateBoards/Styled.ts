...
export const Panel = styled.div<{ $link: string }>`
...
export const PhotoWrapper = styled.div<{ $link: string }>`
  background-image: url(${(props) => props.$link});
  border: none;
  width: 1.5rem;
  height: 1.5rem;
...
export const Photo = styled.div.withConfig({
...
export const DoneIconWrapper = styled.div.withConfig({
  shouldForwardProp: (prop) => prop !== "show",
}) <{ show: boolean }>`
  width: 100%;
  height: 100%;
